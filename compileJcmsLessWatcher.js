...
function compileJCMSLess() {
  ...
  console.log("Starting to watch for modified less files... \n");
  console.log(lessFiles);
  for (var lessItem in lessFiles) {
    if (lessFiles.hasOwnProperty(lessItem)) {
      if (fs.existsSync(path.join(rootPath, lessItem))) {
        fs.watch(path.join(rootPath, lessItem), function (evt, file) {
          console.log(evt, file);

          if (!file || !lessFiles[file]) {
            console.log("File missing :" + file, lessFiles[file] )
            return;
          }
          var result;
          // if (files[file]) {
          //   result = compileLESS(file, lessFiles[file]);
          // }
